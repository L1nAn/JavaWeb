...
2. 创建:定义一个类，实现Servlet接口，并重写接口中所有方法，并在service方法中输入一句话，这个方法会自动执行。
...
3. 配置:在类上使用@WebServlet注解，配置该Servlet的访问路径  （**自己设置就好，但是别忘了/**）
...
* Servlet运行在Servlet容器(web服务器)中，其生命周期由容器来管理，分为4个阶段：
  ...
  2. ==初始化==：在Servlet实例化之后，容器将调用Servlet的==init()==方法初始化这个对象，完成一些如加载配置文件、创建连接等初始化的工作。该方法只==调用一次== 因为servlet对象只会被创建一次。
...
* 通过案例演示下上述的生命周期
  ...
  */
  ...
  ==注意:如何才能让Servlet中的destroy方法被执行？==  强制关闭不会执行。
...
ServletConfig对象，在init方法的参数中有，而Tomcat Web服

务器在创建Servlet对象的时候会调用init方法，必定会传入一个ServletConfig对象，我们只需要将服务器传过来的ServletConfig进行返回即可。具体如何操作?
...
public class ServletDemo5 implements Servlet {
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
       ...
       //1. 获取请求方式,这个请求方式只有在HttpServletRequest才能调用，所以前面要进行强转。
