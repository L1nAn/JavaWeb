...
* table ：定义表格
  ...
  * border：规定表格边框的宽度  为1即可，可以随意设定

  * width ：规定表格的宽度

  * cellspacing：规定单元格之间的空白  为0
...
表单项标签有很多，不同的表单项标签有 不同的展示效果。表单项标签可以分为以下三个：  **提交必须要有name**
...
注：lable中的for  与 input中的 id 对应上  就可以了  达到通过点击用户名来选中输入框
        ...
        注：如果要达到互斥的效果，只要name属性相同就可以了
        如果不设置value的值，那么上传到后台的值就会一样，都会是on，所以我们通过value更改上床的值  男就是1  女就是2 后面的lable和上面的一样
        <br>
        ...
        <br>
        <br>
        <input type="submit" value="免费注册"> //可以改变默认值  通过value
...
* 内联样式：在标签内部使用style属性，属性值是css属性键值对
  ...
  > 给方式只能作用在这一个标签上，如果其他的标签也想使用同样的样式，那就需要在其他标签上写上相同的样式。复用性太差。不常用
...
* 外部样式：定义link标签，引入外部的css文件
  ...
  > 这种方式可以在多个页面进行复用。其他的页面想使用同样的样式，只需要使用 `link` 标签引入该css文件。rel="stylesheet" 是为了解析css文件
