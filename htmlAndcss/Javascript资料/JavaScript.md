...
注：如果是通过外部脚本写入  则输出语句不能写入scripy体内
...
JavaScript 中用 var 关键字（variable (变量）的缩写）来声明变量。格式 `var 变量名 = 数据值;`。而在JavaScript 是一门弱类型语言，变量==可以存放不同类型的值==；如下在定义变量时赋值为数字数据，还可以将变量的值改为字符串类型的数
...
* **number**：数字（整数、小数、NaN(Not a Number)）
  ...
  > ==注意：== NaN是一个特殊的number类型的值，后面用到再说  NaN  和任何数相加 都等于NaN
